...
"""
function get_file(serv, container::String, object_name::String, fname::String; verbose::Bool=false)
...
"""
function get_file(func::Function, serv, container::String, object_name::String; verbose::Bool=false)
	fname = splitdir(object_name)[end]
	mktempdir() do tdir
	    fpathname = joinpath(tdir, fname)
        get_file(serv, container, object_name, fpathname; verbose=verbose)    
        func(fpathname)
    end
end

"""Read IO stream, from Swift, and call func, on it, returning the result """
function get_stream(func::Function, serv, container::String, object_name::String; verbose::Bool=false)
    get_file(serv, container, object_name; verbose=verbose) do fname
		open(func, fname, "r")
    end
end

"""Download a JLD file from Swift. `dataname` is a list of fieldnames to read."""
function get_jld(serv, container::String, object_name::String, datanames...; verbose::Bool=false)
    get_file(serv, container, object_name; verbose=verbose) do fname
